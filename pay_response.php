...
if($isValidChecksum == "TRUE") {
    ...
    if ($_POST["STATUS"] == "TXN_SUCCESS") {
		$upd_query="UPDATE `booking_order` SET `booking_status`='booked',`trans_id`='$_POST[TXNID]',`trans_amt`='$_POST[TXN_AMOUNT]',`trans_status`='$_POST[STATUS]',`trans_resp_msg`='$_POST[RESPMSG]', WHERE `booking_id` ='$slct_fetch[booking_id]'";
        mysqli_query($con,$upd_query);
	}
	else {
		$upd_query="UPDATE `booking_order` SET `booking_status`='payment failed',`trans_id`='$_POST[TXNID]',`trans_amt`='$_POST[TXN_AMOUNT]',`trans_status`='$_POST[STATUS]',`trans_resp_msg`='$_POST[RESPMSG]', WHERE `booking_id` ='$slct_fetch[booking_id]'";
        mysqli_query($con,$upd_query); 
